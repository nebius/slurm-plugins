...
int snccliprecon_ensure_mode(const char *path) {
    ...
    if ((st.st_mode & SNCCLIPRECON_DEFAULT_MODE) == SNCCLIPRECON_DEFAULT_MODE) {
        snccliprecon_log_debug2(
            "ensure_mode: %s already has mode %d",
            path,
            SNCCLIPRECON_DEFAULT_MODE
        );
        return 0;
    }

    snccliprecon_log_debug2(
        "ensure_mode: chmod %d %s", SNCCLIPRECON_DEFAULT_MODE, path
    );
    if (chmod(path, SNCCLIPRECON_DEFAULT_MODE) != 0) {
        if (errno == EPERM || errno == EACCES) {
            snccliprecon_log_debug2(
                "ensure_mode: ignoring permission error for %s", path
            );
            return 0;
        }
        return -1;
