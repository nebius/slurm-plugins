...
int snccliprecon_create_once_per_worker_lock(
    ...
    if (snccliprecon_render_worker_lock_path(
            job_id, step_id, hostname, op, lock_path, sizeof(lock_path)
        ) != 0) {
        return -1;
    }

    int fd =
        open(lock_path, O_CREAT | O_EXCL | O_RDWR, SNCCLIPRECON_DEFAULT_MODE);
    if (fd < 0) {
        if (errno == EEXIST) {
            snccliprecon_log_debug2(
                "worker lock already exists: %s", lock_path
            );
            return 1;
        }
        return -1;
...
void snccliprecon_remove_worker_lock(
    ...
    if (snccliprecon_render_worker_lock_path(
        ...
        snccliprecon_log_debug2_errno(
            errno, "%s: cannot render worker lock", op
        );
        return;
    }
    ...
    if (unlink(lock_path) != 0 && errno != ENOENT) {
        snccliprecon_log_debug2_errno(
            errno, "%s: cannot remove worker lock %s", op, lock_path
        );
        return;
    }
